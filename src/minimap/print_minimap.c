...
void	print_minimap_base(t_main *main)
{
	...
	j = 0;
	while (main->map.array[j])
	{
		i = 0;
		while (main->map.array[j][i])
		{
			if (main->map.array[j][i] == '1')
				print_tile(main, i, j, 0x333333);
			else if (main->map.array[j][i] == '0')
				print_tile(main, i, j, 0xCCCCCC);
			else if (main->map.array[j][i] == '2')
				print_tile(main, i, j, 0x4a2514);
			else if (main->map.array[j][i] == '3')
